...
@Path("/singlelistingdatastoreget")
public class single_listing_datastoreservice {
	...
	public Response getproperty() throws JSONException {
		...
		List<AddProperty> addprop = objectify.query(AddProperty.class).filter("property_price !=", null).list();
		...
		for (AddProperty prop: addprop) {
			...
			addproperty.setProperty_expriry_date(prop.property_expriry_date);
