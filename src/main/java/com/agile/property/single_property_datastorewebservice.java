...
@Path("/singledatastoreget")
public class single_property_datastorewebservice {
	...
	public Response getproperty(@QueryParam("id") Long id) throws JSONException {
		System.out.println("I am here");
		Objectify objectify=ObjectifyService.begin();
		JSONObject jsonObject = new JSONObject();
		//List<AddProperty> addprop = objectify.query(AddProperty.class).list();
		//Query q = objectify.query(AddProperty.class)
		List<AddProperty> addprop = objectify.query(AddProperty.class).filter("id", id).list();
		...
		for (AddProperty prop: addprop) {
			...
			addproperty.setProperty_expriry_date(prop.getProperty_expriry_date());
    ...
    public Response add(String incomingData) throws IOException
     {
		...
		try {
			...
			addproperty.setProperty_expriry_date(message.property_expriry_date);
	    	...
	    	if(message.id!=null){
	    		Key<AddProperty> key = Key.create(AddProperty.class,message.id);
	    		addproperty = objectify.get(key);
	    		System.out.println("Key:"+key);
	    	}	    	
			ll.add(addproperty);
			objectify.put(ll);
